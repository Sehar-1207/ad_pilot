...
export function ConnectionAlert({ isConnected }: ConnectionAlertProps) {
  ...
  return (
    <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
      ...
      <button className="text-xs font-extrabold text-amber-600 dark:text-amber-400 bg-amber-500/15 hover:bg-amber-500/25 px-4 py-2 rounded-xl transition-colors flex-shrink-0 border border-amber-500/20">
        Connect Now
      </button>
    </div>
  );
