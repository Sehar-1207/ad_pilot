...
export function CampaignsTable({ campaigns, userPlan }: CampaignsTableProps) {
  return (
          ...
          <tbody className="divide-y divide-[var(--border-color)] text-xs font-semibold text-[var(--text-primary)]">
            {campaigns.map((c, idx) => (
              <tr key={idx} className="hover:bg-[var(--bg-accent)] transition-colors">
                ...
                <td className="py-4 px-6 text-[var(--text-primary)]">{c.spend}</td>
                <td className="py-4 px-6 text-[var(--text-primary)]">{c.clicks}</td>
                <td className="py-4 px-6 text-[var(--text-primary)]">{c.ctr}</td>
                <td className="py-4 px-6 font-bold text-[var(--accent-teal)]">{c.roas}</td>
              </tr>
            ))}
          </tbody>
        </table>
