...
const normalizeInstagramAccounts = (response: unknown): InstagramAccount[] => {
  const data = extractData(response);
  if (Array.isArray(data)) {
    return data;
  }
...
const normalizeConnectedInstagram = (response: unknown): ConnectedInstagramAccount | null => {
  const data = extractData(response);
  if (!data) {
    return null;
  }
...
const getAccountId = (response: unknown): string | null => {
  const data = extractData(response);
  return (data?.adAccountId ?? data?.metaAdAccountId ?? data?.accountId ?? null);
};

const getTokenExpiry = (response: unknown): string | null => {
  const data = extractData(response);
  return (data?.tokenExpiresAt ?? data?.metaTokenExpiresAt ?? data?.expiresAt ?? null);
};
...
export default function SettingsPage() {
  ...
  const [connectingMeta, setConnectingMeta] = useState<boolean>(false);
  ...
  const [connectingInstagram, setConnectingInstagram] = useState<string | null>(null);
  const [syncingInstagram, setSyncingInstagram] = useState<boolean>(false);
  const [disconnectingInstagram, setDisconnectingInstagram] = useState<boolean>(false);
  ...
  const loadMeta = async (): Promise<void> => {
    try {
      ...
      accounts.forEach((account) => {
        states[account.id] =
          account.isEnabled ?? account.enabled ?? account.id === accountId;
      });
  ...
  const loadInstagram = async (): Promise<void> => {
    try {
      ...
      setInstagramAccounts(normalizeInstagramAccounts(accountsResponse));
      setConnectedInstagram(normalizeConnectedInstagram(connectedResponse));
    } catch (error) {
  ...
  const loadAll = async (): Promise<void> => {
    await Promise.all([loadMeta(), loadInstagram(), loadProfile()]);
  };
  ...
  const handleConnectMeta = (): void => {
    try {
      ...
      toast.error(getErrorMessage(error, "Unable to connect Meta."));
    }
  };
  ...
  const handleSyncMeta = async (): Promise<void> => {
    ...
    try {
      ...
      toast.error(getErrorMessage(error, "Meta sync failed."));
    } finally {
  ...
  const handleConnectAdAccount = async (
    ...
    try {
      ...
      toast.error(getErrorMessage(error, "Failed to connect ad account."));
    } finally {
  ...
  const handleToggleAdAccount = async (
    ...
    try {
      ...
      toast.error(getErrorMessage(error, "Failed to update ad account."));
    }
  };
  ...
  const handleConnectInstagram = async (
    ...
    try {
      ...
      toast.error(getErrorMessage(error, "Failed to connect Instagram."));
    } finally {
  ...
  const handleSyncInstagram = async (): Promise<void> => {
    ...
    try {
      ...
      toast.error(getErrorMessage(error, "Instagram sync failed."));
    } finally {
  ...
  const handleDisconnectInstagram = async (): Promise<void> => {
    try {
      ...
      toast.error(getErrorMessage(error, "Failed to disconnect Instagram."));
    } finally {
  ...
  const accountName = selectedAccount?.name ?? selectedMetaAccountId ?? null;

  const getTokenStatus = (): { text: string; valid: boolean } => {
    if (!metaConnected) {
      return { text: "Not connected", valid: false };
    }
    if (!tokenExpiresAt) {
      return { text: "Connected", valid: true };
    }
      ...
      <div className="border-b border-[var(--border-color)]">
        <div className="flex gap-6 overflow-x-auto">
          <button
            ...
            className={`relative whitespace-nowrap pb-3 text-sm font-medium transition-colors ${activeTab === "integrations"
          ...
          <button
            ...
            className={`relative whitespace-nowrap pb-3 text-sm font-medium transition-colors ${activeTab === "preferences"
              ? "text-[var(--text-primary)]"
              : "text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
              }`}
          >
          ...
          <button
            ...
            className={`relative whitespace-nowrap pb-3 text-sm font-medium transition-colors ${activeTab === "billing"
              ? "text-[var(--text-primary)]"
              : "text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
              }`}
          >
